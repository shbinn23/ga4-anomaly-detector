...
This dashboard is an operations workspace for GA4 anomaly detection.

- Prefer document-like hierarchy over flashy BI visuals.
- Prioritize anomaly readability over decorative visuals.
- Use muted surfaces and restrained contrast.
- Forecast charts must always show:
  - actual value
  - forecast value
  - confidence interval
  - anomaly points
- Tables should be compact and scannable.
- Anomaly states should be visible but not alarmist.
- Do not use official Notion branding, logos, or product names.
- Use the design language as inspiration only.